class Validators {
  ...
  static final Set<String> _dialCodes = {
    ...
    '689', '690', '691', '692', '850', '852', '853', '855', '856', '870', '878',
    '880', '881', '882', '883', '886', '960', '961', '962', '963', '964', '965',
    '966', '967', '968', '970', '971', '972', '973', '974', '975', '976', '977',
    '979', '991', '992', '993', '994', '995', '996', '998',
  };
  ...
  static Map<String, String>? parsePhoneNumber(String value) {
    final raw = value.replaceAll(RegExp(r'[\s\-\(\)]'), '');
    if (raw.isEmpty) return null;

    if (raw.startsWith('+')) {
      final digits = raw.substring(1);
      if (!RegExp(r'^\d{4,15}$').hasMatch(digits)) {
        return null;
      }

      for (final length in [1, 2, 3]) {
        if (digits.length <= length) continue;
        final dialCode = digits.substring(0, length);
